-- Add swimming terms to the database (fixed version)
...
BEGIN
    ...
    (gen_random_uuid(), 'Stroke Recovery', 'stroke-recovery', 'Non-propulsive phase when arm returns to starting position', 'intermediate', ARRAY['Keep your recovery relaxed', 'High elbow recovery is efficient'], NOW(), NOW()),
    ...
    (gen_random_uuid(), 'Easy Swimming', 'easy-swimming', 'Low-intensity swimming', 'beginner', ARRAY['Warm up with 400 easy', 'Easy pace should feel comfortable'], NOW(), NOW()),
    (gen_random_uuid(), 'Recovery Swimming', 'recovery-swimming', 'Low-intensity swimming for active rest', 'beginner', ARRAY['Do 200 recovery between sets', 'Recovery swimming aids lactic acid clearance'], NOW(), NOW()),
    ...
    -- Handle duplicates for both name and slug
    ...
    FROM terms t 
    WHERE t.slug IN (
        'stroke', 'front-crawl', 'individual-medley', 'open-turn', 'streamline', 'catch', 'stroke-recovery',
        'bilateral-breathing', 'sighting', 'negative-split', 'build', 'descending', 'pull-buoy', 'kickboard',
        'paddles', 'fins', 'snorkel', 'tapering', 'shave-down', 'tech-suit', 'hypoxic-training',
        'easy-swimming', 'recovery-swimming', 'aerobic-base', 'lactate-threshold', 'distance-per-stroke',
